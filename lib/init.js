...
function buildPrompt(root, shortcutDescriptions = []) {
  ...
  const shortcutsAsk = shortcutDescriptions.length
    ? `\n\nThe user asked for these shortcuts specifically — turn each into a shortcuts[] entry (pick a sensible, unused single-character key for each; use "restart": "<service>" for a restart-type shortcut, or "command": "<shell command>" for anything else):\n${shortcutDescriptions.map((d) => `- ${d}`).join('\n')}`
    : ''
...
// customized vibestackr's own entry (extra env, a different invocation) —
// and does nothing if a 'vibestackr' entry already exists, same reasoning.
function ensureMcpConfig(root) {
  ...
  if (fs.existsSync(mcpPath)) {
    try {
      ...
      console.error(`[vibestackr] ${mcpPath} exists but isn't valid JSON — leaving it as-is (${err.message})`)
      return
    }
  }
  existing.mcpServers = existing.mcpServers || {}
  if (existing.mcpServers.vibestackr) return // already configured — don't clobber a user's own tweaks

  existing.mcpServers.vibestackr = { command: 'npx', args: ['vibestackr', 'mcp'] }
  fs.writeFileSync(mcpPath, `${JSON.stringify(existing, null, 2)}\n`)
  console.error(`[vibestackr] wrote '${mcpPath}' — registers 'vibestackr mcp' as a project-scoped MCP server`)
}
...
// Documents vibestackr's presence + its configured shortcuts in AGENTS.md, so
...
// previous `vibestackr init` run) — nothing to document otherwise. Re-running
// replaces the previous vibestackr:start/end block in place rather than
...
async function main(argv = []) {
  ...
  if (!cli) {
    ...
    if (await confirm('Register vibestackr as an MCP server in .mcp.json?')) ensureMcpConfig(root)
...
async function configMain(argv = []) {
  ...
  if (!CANDIDATES.some((f) => fs.existsSync(path.join(root, f)))) {
    console.error(`[vibestackr] no config (${CANDIDATES.join(', ')}) found in ${root} — run 'vibestackr init' first.`)
    process.exit(1)
  }
...
module.exports = { main, configMain, buildPrompt, buildUpdatePrompt, detectCli, findCli, ensureMcpConfig, ensureAgentsMd, buildAgentsSection, runAgent, tail, confirm, isInteractive, collectShortcutDescriptions, CLI_INVOCATIONS }
