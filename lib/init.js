...
function buildPrompt(root) {
  ...
  return `You're setting up vibestackr for this repository. vibestackr is a TUI that brings up a local dev stack (multiple services, in dependency order, with liveness checks) from a single config file: .vibestackr.yaml (or .vibestackr/.vibestackr.yml/.vibestackr.json/.vibestackr) in the project root.
