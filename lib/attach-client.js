...
// shape lib/ui.js already reads (`status`, `included()`, `runShortcut()`,
// `restartService()`) for
...
// bin/vibestackr for how a daemon gets started/found.
//
...
// response, connection closed) — same design already used by `vibestackr
...
function createAttachClient({ config, root, onServicesChanged }) {
  ...
  // observed — calls onServicesChanged() so the caller (bin/vibestackr) can
  ...
  return {
    ...
    startPolling,
    stopPolling,
    runShortcut: (shortcut, values) => requestSocket(root, 'run_shortcut', { key: shortcut.key, inputs: values }),
    restartService: (name) => requestSocket(root, 'restart', { name }),
    ...
    // nothing to poll for, so the caller (bin/vibestackr) needs to surface
    // that itself.
