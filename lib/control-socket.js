...
// The engine-facing side of the daemon/attach architecture (see bin/vibestackr
...
// started, reattached, or `vibestackr mcp`) is a separate process that talks
...
const NOT_RUNNING_MESSAGE = "vibestackr isn't running for this project — start it with `npx vibestackr` first"
...
// inside the project (e.g. `<root>/.vibestackr/control.sock`) can blow past
// that on an unremarkable deeply-nested checkout. Keeping this fixed-length
// and root-independent avoids the limit entirely.
//
// Deliberately os.homedir() (HOME), not os.tmpdir() (TMPDIR): `vibestackr mcp`
...
// `vibestackr mcp` compute two *different* base directories (e.g. macOS's
...
function socketPath(root) {
  return path.join(os.homedir(), '.vibestackr', 'sockets', `${projectHash(root)}.sock`)
}
...
function daemonLogPath(root) {
  return path.join(os.homedir(), '.vibestackr', 'daemon-logs', `${projectHash(root)}.log`)
}
...
// exactly one response line, and closes. Shared by `vibestackr mcp`
// (lib/mcp-server.js), the attach client (lib/attach-client.js), and
// bin/vibestackr's own `stop` subcommand, rather than each reimplementing it.
...
async function dispatch(method, params, { engine, config, onQuit, onReloadConfig }) {
  switch (method) {
    ...
    // last-N-lines snapshot, what `vibestackr mcp`'s get_logs tool uses) so
    ...
    // is purely runtime state) — what `vibestackr mcp`'s get_services tool
    ...
    // changes, like nodemon or vite HMR — informational only, vibestackr
    ...
    case 'shortcuts':
      // `inputs` (if any) is what makes list_shortcuts useful to an agent
      // over MCP — without it there'd be no way to know a shortcut expects
      // run_shortcut's `inputs` parameter at all, let alone which names it
      // reads out of it.
      return {
        shortcuts: (config.shortcuts || []).map((s) => ({
          key: s.key,
          label: s.label,
          inputs: (s.inputs || []).map((i) => ({ name: i.name, label: i.label ?? i.name, placeholder: i.placeholder ?? null, default: i.default ?? null })),
        })),
      }
    case 'run_shortcut': {
      ...
      await engine.runShortcut(shortcut, params.inputs || {})
      return { ok: true }
    }
    ...
    // what that does/doesn't do) — `onReloadConfig` (bin/vibestackr) owns the
...
// Starts listening, or returns `{ ok: false, reason }` if another vibestackr
