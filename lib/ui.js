...
function createUI({ config, engine, onQuit, onDetach, onReloadConfig }) {
  ...
  screen.key(['m'], () => {
    ...
    // Not screen.program.enableMouse() — that re-guesses encoding from $TERM
    // (see the sgrMouse comment above, line ~31) and can pick a different one
    // than what we forced at startup, permanently breaking click/scroll for
    // terminals it guesses wrong for (e.g. Hyper) once toggled off then back on.
    if (mouseEnabled) screen.program.setMouse({ sgrMouse: true }, true)
    screen.render()
  })
