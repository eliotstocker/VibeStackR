...
function createUI({ config, engine, onQuit, onDetach, onReloadConfig }) {
  ...
  // already-detached process — see bin/vibestakr) running untouched. 'q'
  ...
  // stack.
  // Same pink-to-orange gradient as the VIBESTAKR logo, run across the whole
  ...
  const redrawStatus = () => {
    ...
    // rebuilds this whole UI from scratch, see bin/vibestakr, rather than
    ...
    const brandSegments = [{ text: 'VibeStakR', bold: true }, { text: ' | ' }]
