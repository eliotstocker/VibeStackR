...
function createEngine({ config, args }) {
  ...
  // bin/vibestakr's reload handler, which is what actually re-reads the
  ...
  // service someone's actively relying on; restart_service/`vibestakr stop`
  ...
  function reloadConfig(newConfig) {
    ...
    for (const name of oldNames) {
      if (!newNames.has(name) && children.has(name)) {
        warn(`${name} was removed from config but is still running — restart_service won't find it anymore; stop it via its own process, or 'vibestakr stop' to clear everything`)
      }
    }
    UI.refreshStatus()
    log('config reloaded')
