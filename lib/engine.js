...
function pythonInstallCommand(cwd) {
  if (fs.existsSync(`${cwd}/uv.lock`)) return { command: 'uv', args: ['sync'] }
  if (fs.existsSync(`${cwd}/poetry.lock`)) return { command: 'poetry', args: ['install'] }
  ...
  return { command: 'poetry', args: ['install'] } // only a pyproject.toml with no lockfile — poetry install both resolves and locks
}
...
const INSTALL_STEPS = {
  ...
  rust: { reason: rustInstallReason, marker: '.vibestackr-rust-installed', label: 'cargo', build: () => ({ command: 'cargo', args: ['fetch'] }) },
  python: { reason: pythonInstallReason, marker: '.vibestackr-python-installed', label: 'python', build: pythonInstallCommand },
}
...
function interpolateShortcutInputs(command, inputs, values) {
  ...
  for (const input of inputs) {
    const raw = values?.[input.name] ?? input.default ?? ''
  ...
  function spawnService(service) {
    ...
    if (service.type === 'node') {
      ...
      if (reason) {
        ...
        const { command, args: cmdArgs } = step.build(service.cwd)
...
module.exports = { createEngine, STATE_COLOR, STATE_GLYPH, goInstallReason, rustInstallReason, pythonInstallReason, pythonInstallCommand, interpolateShortcutInputs }
