...
consumer-supplied `.vibestackr.yaml` (or `.vibestackr.yml`/`.vibestackr.json`/
`.vibestackr`, or an explicit `--config <path>`), not in this repo.
`vibestackr` is just the generic runner: spawn services, stream their
...
own — see "Daemon / attach architecture" below. `npx vibestackr` either
...
or the `b` shortcut) and reattaching (running `npx vibestackr` again) the same
underlying mechanism rather than two separate features.

This repo ships the runner only. There is no `.vibestackr.yaml` here — it's
provided by whatever project drops `vibestackr` into itself.

## Layout
- `bin/vibestackr` — entry point. Routes subcommands: no arg → start-or-attach
...
- `lib/config.js` — finds and loads `.vibestackr.(json|yaml|yml)`/`.vibestackr`
  ...
  all (plain `.vibestackr`) both parse as JSON.
- `lib/engine.js` — the orchestration engine: `status`/`children` maps,
  ...
  daemon process (`bin/vibestackr --daemon`) — a TUI never holds one directly.
...
- `lib/control-socket.js` — the engine-facing side of the MCP integration.
  ...
  `bin/vibestackr`'s own `stop` subcommand, rather than each reimplementing
...
- `lib/mcp-server.js` — `vibestackr mcp`, a thin stdio MCP client for the
  socket above. Never touches the engine directly.
- `lib/init.js` — `vibestackr init`, agent-assisted config generation. Builds
  a prompt (embeds the JSON Schema + an annotated example), detects a coding
  CLI on `PATH`, and hands the prompt off to it. See "vibestackr init" below.
...
- **Distributed and run via `npx vibestackr`** (or as a `devDependency` +
...
- **Everything stack-specific is config, not code.** Adding a service, a
  ...
  project's `.vibestackr.json` schema (`services[]`, `shortcuts[]`,
  `dependencies[]`, `warnings[]`), not hardcoded into `vibestackr`. If a task
...
- **Tests exist for `lib/config.js`, `lib/engine.js`, `lib/json-log.js`,
  ...
  actually gone) — SIGTERM to the spawned `vibestackr` process now only
...
`bin/vibestackr` splits into two roles that are always separate OS processes,
never combined in one:
- **The daemon** (`bin/vibestackr --daemon`, a hidden flag) — owns
...
- **A client** — either a TUI (`lib/ui.js` + `lib/attach-client.js`) or
  `vibestackr mcp` (`lib/mcp-server.js`). Cheap, disposable, talks to the
...
separate mechanisms:
- **`npx vibestackr`** checks whether a daemon is already running for this
...
- **Reattaching** is just running `npx vibestackr` again — since a daemon was
...
- **The `b` shortcut** (`lib/ui.js`) calls `attach.detach()` — stops this
  ...
  stack.
- **`vibestackr stop`** does exactly what `quit()` does, from a one-shot
...
- `--exclude`/`--only`/`--persist-logs`/`--service-log` only matter at the
  ...
  vibestackr` prints a warning if any of them are passed while attaching to
...
`~/.vibestackr/sockets/<sha1(realpath(root))>.sock` exposing the running
...
`vibestackr mcp`'s `get_logs` tool uses), `tail` (cursor-based — `{name,
...
- `requestSocket(root, method, params)` (also in `lib/control-socket.js`) is
  ...
  `lib/attach-client.js`, and `bin/vibestackr`'s own `stop` subcommand, rather
...
- **The socket lives at `~/.vibestackr/sockets/<sha1(realpath(root))>.sock` —
  ...
  - A project-nested path (`<root>/.vibestackr/control.sock`) can exceed the
  ...
  - `os.tmpdir()` depends on `TMPDIR`. `vibestackr mcp` is normally launched
    ...
    tmpdir() made the daemon and `vibestackr mcp` compute two different,
...
- **A socket bind failure is fatal to the daemon** (`bin/vibestackr`'s
  ...
  `vibestackr stop`, or `vibestackr mcp`); a daemon nobody can ever talk to
